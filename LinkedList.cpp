...
void deleteNode(NodePtr& Head, int item)
{
	...
	{
		cout << "Delete error: " << item << " not in list!" << endl;
		return;
	}
	if(cur==Head)
	Head = Head->next;
...
{  
	...
	else res=addEndFunc(reverse(head->next),head->data);
    return res;
} 
...
{
	bool palin;
	if (p1==NULL) palin = true;
	else if ((p1->data)!=(p2->data)) palin = false;
	else palin=palindrome(p1->next,p2->next);
    return palin;	
}
...
main()
{
	printf("Program to create LinkedList\n");
	printf("Author:-Aditya Pawar\n");
	printf("Reg.No:-21BCE11104\n");
	printf("Experiment Date:- 21-03-2022\n");
